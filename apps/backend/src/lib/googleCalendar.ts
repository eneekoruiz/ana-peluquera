...
export interface CreateAppointmentRequest {
  start: Date; end: Date; serviceId: string; bookingId: string; notes?: string; timeZone?: string; phase1Min?: number; phase2Min?: number; phase3Min?: number;
}
...
async function getAdminSettings(): Promise<AdminSettings> {
  const defaultSettings: AdminSettings = {
    ...
    bookingsEnabled: true, todayClosed: false, todayClosedDate: null, vacationRanges: []
  };
  try {
    ...
    const workers = staffArray.map((worker: any) => ({
      id: worker.id, name: worker.name || "Sin Nombre", daysOfWeek: worker.workingDays || [], skills: worker.skills || []
    }));
    return {
      workers: workers.length > 0 ? workers : defaultSettings.workers,
      bookingsEnabled: data.bookings_enabled !== false,
      todayClosed: !!data.today_closed, todayClosedDate: data.today_closed_date || null, vacationRanges: data.vacation_ranges || []
    };
...
function getGoogleAuthClient(): any {
  ...
  try {
      let creds;
      if (jsonPath.trim().startsWith('{')) { creds = JSON.parse(jsonPath); } 
      else { creds = JSON.parse(readFileSync(path.resolve(process.cwd(), jsonPath), "utf8")); }
    ...
    return jwtClient;
  } catch (error) { throw new Error("No se pudo leer el JSON de Google Calendar."); }
}
...
// 🚀 EL CORAZÓN DEL "FAKE UTC": Obligamos al servidor a congelar la hora exacta de España en formato UTC
function toDateTime(value?: string): Date | null {
  if (!value) return null;
  if (!value.includes('T')) { return new Date(`${value}T00:00:00Z`); }
  const localStr = value.substring(0, 19); 
...
export async function getBusySlots(dateRange: CalendarDateRange): Promise<{ busy: BusyInterval[] }> {
  ...
  const res = await calendar.events.list({
    calendarId, timeMin: dateRange.start.toISOString(), timeMax: dateRange.end.toISOString(), singleEvents: true, orderBy: "startTime", maxResults: 2500,
  });
  ...
  for (const event of res.data.items ?? []) {
    ...
    // 🚀 Extracción limpia del día sin conversiones de zona horaria
  ...
  for (const [dateString, intervals] of Object.entries(intervalsByDay)) {
      ...
      // Creamos la fecha local para saber qué día de la semana es (Lunes, Martes...)
      const localDateForCapacity = new Date(y, m - 1, d);

      if (isDayClosed(dateString, settings)) {
          finalBusyIntervals.push({ start: new Date(`${dateString}T00:00:00Z`), end: new Date(`${dateString}T23:59:59Z`), sourceEventId: "closed_by_admin" });
          continue;
      }
...
export async function createAppointment(req: CreateAppointmentRequest & { customerName?: string }): Promise<{ eventId: string; htmlLink?: string; }> {
  ...
  const settings = await getAdminSettings();
  if (isDayClosed(dateString, settings)) throw new Error("La peluquería está cerrada en esta fecha.");
  ...
  const event = await calendar.events.insert({
    ...
    requestBody: {
      ...
      extendedProperties: {
        private: { system: SYSTEM_MARKER, bookingId: req.bookingId, serviceId: req.serviceId, workerName: assignedTo, phase1Min: req.phase1Min != null ? String(req.phase1Min) : "", phase2Min: req.phase2Min != null ? String(req.phase2Min) : "", phase3Min: req.phase3Min != null ? String(req.phase3Min) : "" },
      },
    },
  });
